...
export default function MovementMapScreen() {
  ...
  const fixedCorePlan = reviewGate?.rehab_plan_source === "fixed_core_programme";
              ...
              <View style={styles.expandedDetails} testID="movement-map-expanded-details">
                ...
                <View style={[styles.planSummary, !isWide && styles.planSummaryNarrow]}>
                  <View style={styles.planSummaryCopy}>
                    ...
                    <Text style={styles.planTitle}>{canViewPlan ? (fixedCorePlan ? "Your core rehab plan is ready" : "Your rehab plan is ready") : noRehabNeeded ? "No rehab plan is needed" : "Your plan is waiting for review"}</Text>
                    <Text style={styles.planText}>{canViewPlan ? (fixedCorePlan ? "The same four core exercises are assigned during testing; your exercise frequency sets the suggested difficulty." : "Your plan focuses on the movement areas that need support.") : reviewGate?.patient_message}</Text>
                  </View>
